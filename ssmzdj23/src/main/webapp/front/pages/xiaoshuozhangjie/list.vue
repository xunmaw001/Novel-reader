	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
        ...
        <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			...
			<view v-if="queryIndex==0" class="search-form round">
				...
				<input v-model="searchForm.xiaoshuomingcheng" type="text" placeholder="小说名称" ></input>
			</view>
		...
		<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"#efefef"}'>
			<view @tap="onDetailTap(product)" class="uni-product" :style='{"borderRadius":"8rpx","backgroundColor":"rgba(255, 255, 255, 1)"}' v-for="(product,index) in list" :key="index">
								                				                                <view class="uni-product-title" :style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"#333","textAlign":"left"}'>{{product.xiaoshuomingcheng}}</view>
								                                <view class="image-view">
                    <image :style='{"borderRadius":"8rpx","width":"100%","height":"100%"}' mode="aspectFill" class="uni-product-image" :src="product.tupianfengmian?product.tupianfengmian.split(',')[0]:''"></image>
                </view>
                				                				                				                				                				                												<view style="display: flex;justify-content: space-between;">
					<text v-if="isAuth('xiaoshuozhangjie','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
					<text v-if="isAuth('xiaoshuozhangjie','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
				</view>
			</view>
		</view>
		
		
		<button v-if="isAuth('xiaoshuozhangjie','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.xiaoshuomingcheng="";
			...
			async upCallback(mescroll) {
				...
				let res = await this.$api.list(`xiaoshuozhangjie`, params);
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('xiaoshuozhangjie', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				let searchForm = {
					...
					limit: this.mescroll.size
																				...
																				if(this.searchForm.xiaoshuomingcheng){
					searchForm['xiaoshuomingcheng'] = '%' + this.searchForm.xiaoshuomingcheng + '%'
				}	
																																								if(this.searchForm.yonghuxingming){
					searchForm['yonghuxingming'] = '%' + this.searchForm.yonghuxingming + '%'
				}	
																												let res = await this.$api.list(`xiaoshuozhangjie`, searchForm);
				// 如果是第一页数据置空
