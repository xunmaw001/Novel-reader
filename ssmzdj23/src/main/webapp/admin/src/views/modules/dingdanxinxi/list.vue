<template>
  <div class="main-content">
		     ...
		     <template slot-scope="scope">
              ...
              <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('dingdanxinxi','查看') && contents.tableBtnIcon == 0" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'详情':'' }}</el-button>
                                                                <el-button v-if="isAuth('dingdanxinxi','推送章节') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="xiaoshuozhangjieCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'推送章节':'' }}</el-button>
                <el-button v-if="isAuth('dingdanxinxi','推送章节') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="xiaoshuozhangjieCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'推送章节':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('dingdanxinxi','推送章节') && contents.tableBtnIcon == 0" type="success" size="mini" @click="xiaoshuozhangjieCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'推送章节':'' }}</el-button>
    ...
    <add-or-update v-if="addOrUpdateFlag" :parent="this" ref="addOrUpdate"></add-or-update>
            ...
            <xiaoshuozhangjie-cross-add-or-update v-if="xiaoshuozhangjieCrossAddOrUpdateFlag" :parent="this" ref="xiaoshuozhangjieCrossaddOrUpdate"></xiaoshuozhangjie-cross-add-or-update>
...
import xiaoshuozhangjieCrossAddOrUpdate from "../xiaoshuozhangjie/add-or-update";
export default {
  data() {
    return {
                                                                                                                                                                                                                                                                                                      searchForm: {
      ...
      addOrUpdateFlag:false,
                  xiaoshuozhangjieCrossAddOrUpdateFlag: false,
  ...
  components: {
    AddOrUpdate,
            xiaoshuozhangjieCrossAddOrUpdate,
          },
  methods: {
    contentStyleChange() {
    ...
    contentPageStyleChange(){
      ...
      this.contents.pageEachNum = 10
            ...
            xiaoshuozhangjieCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.xiaoshuozhangjieCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','dingdanxinxi');
      this.$nextTick(() => {
        this.$refs.xiaoshuozhangjieCrossaddOrUpdate.init(row.id,type);
      });
    },
                payHandler(row){
    ...
    shDialog(row){
      ...
      if(row){
        this.shForm = {
                    ...
                    xiaoshuobianhao: row.xiaoshuobianhao,
                    xiaoshuomingcheng: row.xiaoshuomingcheng,
                    xiaoshuofenlei: row.xiaoshuofenlei,
